...
musicToggleBtn.textContent = '🔇'; // Set initial icon


musicToggleBtn.addEventListener('click', () => {
    
    if (!isMusicStarted) {
        // This is the FIRST time the user clicks the button
        backgroundMusic.play();   // Start the music
        backgroundMusic.muted = false;
        isMusicStarted = true;    // Set flag to true
        musicToggleBtn.textContent = '🔊'; // Change icon to "on"
    } else {
        // The music is already playing, so just toggle mute/unmute
        if (backgroundMusic.muted) {
            backgroundMusic.muted = false;
            musicToggleBtn.textContent = '🔊'; // Change icon to "on"
        } else {
            backgroundMusic.muted = true;
            musicToggleBtn.textContent = '🔇'; // Change icon to "off"
        }
    }
});